...
public class SharedMem extends IPC {
  ...
  static final int SHM_RDONLY = 010000;	
  /** Round attach address to SHMLBA. */
  static final int SHM_RND = 020000;	
  /** The modulus for attaching shared memory by logical address.  This
      ...
      system dependent, it can be handled by configuration or property
      settings.
   */
  public static final long SHMLBA = getLBA();

  public static class shmid_ds extends IPC.Perm {
    shmid_ds(int id) throws IPCException { // only we can create
      super(id);
  ...
  /** Attach the shared memory for read/write at any available logical address.
   While posix supports attaching at a specific address (within
   system dependent contraints), I cannot see any utility for this
   in a Java program - so it is not supported for now.

   FIXME: support attaching readonly.  Support detaching and reattaching
   at the same address to change readonly status.
   @return a {@link posix.CPtr} to the attached memory segment
   */
