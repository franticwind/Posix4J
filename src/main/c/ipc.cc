/*
 * $Log: ipc.cc,v $
 * Revision 1.14  2009/09/13 03:43:30  stuart
 * Document 1.1.9, 1.2 API changes.  Rename semctlb to work around javah bug.
 *
 * Revision 1.13  2009/09/13 03:07:47  stuart
 * Sig change
 *
 * Revision 1.12  2009/09/13 01:56:20  stuart
 * Support IPC.setPerm().
 *
 * Revision 1.11  2009/08/17 21:08:33  stuart
 * Update CVS comment leaders on cc files.
 *
...
JNIEXPORT jint JNICALL Java_posix_MsgQ_msgctl
  (JNIEnv *env, jclass c, jint qid, jint cmd, jobject buf) {
    ...
    int rc;
    if (buf != 0) {
      if (cmd == posix_MsgQ_IPC_SET) {
	ds.getmsqid(env,&mbuf,buf);
	rc = msgctl(qid,IPC_SET,&mbuf);
      }
      else if (cmd == posix_MsgQ_IPC_STAT) {
...
JNIEXPORT jint JNICALL Java_posix_SharedMem_shmctl
  (JNIEnv *env, jclass, jint id, jint cmd, jobject obj) {
    ...
    int rc = -1;
    if (obj != 0) {
      if (cmd == posix_SharedMem_IPC_SET) {
	shmds.getshmid(env,&buf,obj);
	rc = shmctl(id,IPC_SET,&buf);
      }
      else if (cmd == posix_SharedMem_IPC_STAT) {
	rc = shmctl(id,IPC_STAT,&buf);
	shmds.setshmid(env,&buf,obj);
      }
    }
    else if (cmd == posix_SharedMem_IPC_RMID)
...
JNIEXPORT jint JNICALL Java_posix_SemSet_semctlb
  ...
  else if (cmd == posix_SemSet_IPC_RMID)
